...
We'll use {MyLink("https://docs.servant.dev/en/stable/", true, "Servant")} to create our endpoints and one may understand it by following their easy to understand tutorial {MyLink("https://docs.servant.dev/en/stable/tutorial/index.html", true, "here")}.
...
Currently Atlas supports the following providers (& it would be highly appreciated if community enriches this by contributing to {MyLink(`${atlasRepoURL}`, true, 'Atlas')}):

* {MyLink("https://www.gomaestro.org/", true, "Maestro")}.
* Locally ran node.
...
<Callout>
  We have given a sample `config.json` file {MyLink(`${exampleProjectURL}/config.sample.json`, true, "here")}.
</Callout>
...
* **`logging`**: Provided list gets folded over (in Katip's {MyLink("https://hackage.haskell.org/package/katip-0.8.7.2/docs/Katip.html#t:LogEnv", true, "logEnv", true)}) and its parameter corresponds to that in {MyLink("https://hackage.haskell.org/package/katip", true, "Katip")}. Its parameters (like `severity`, `verbosity`) and its general usage can be understood by going over their official haddock documentation {MyLink("https://hackage.haskell.org/package/katip-0.8.7.2/docs/Katip.html", true, "here")}. Katip is also explained in {MyLink("https://link.springer.com/book/10.1007/978-1-4842-3739-7", true, "this")} book on web development in Haskell.
* **`utxoCacheEnable`**: Enabling this boolean will enable cache (using {MyLink("https://hackage.haskell.org/package/cache-0.1.3.0/docs/Data-Cache.html", true, "Data.Cache", true)}) whereby queries related to fetching UTxOs won't generate call to provider if the entry exists in cache (& has not yet expired).

### Parsing Given Configuration

The file {MyLink(`${exampleProjectURL}/server/server-main.hs`, true, "server-main.hs", true)} fires up our server. It reads & parses the configuration file and using it makes our endpoints (which we will define shortly) available. Here is its entire code.
...
Entire code for it is available {MyLink(`${exampleProjectURL}/server-lib/BetRef/Api/Context.hs`, true, "here")}
</Callout>
...
<Callout type="warning" emoji="📃">
Entire code for it is available {MyLink(`${exampleProjectURL}/server-lib/BetRef/Api/Tx.hs`, true, "here")}
</Callout>
...
<Callout type="warning" emoji="📃">
Entire code for it is available {MyLink(`${exampleProjectURL}/server-lib/BetRef/Api/BetRef.hs`, true, "here")}
</Callout>
...
<Callout>
Do note that we can also sign the transactions in server using the `signTx` function defined in {MyLink(`${atlasRepoURL}/src/GeniusYield/Types/TxBody.hs`, true, "TxBody.hs", true)}
</Callout>

### Wrap-Up

Our both the endpoints file (transaction submition & transaction building) our wrapped up in our {MyLink(`${exampleProjectURL}/server-lib/BetRef/Api/Api.hs`, true, "Api.hs", true)} following the usual servant boilerplate.
...
Now coming back to our {MyLink(`${exampleProjectURL}/server/server-main.hs`, true, "server-main.hs", true)} file, we can now understand the highlighted code sections which relates to obtaining the Swagger file (generated from `apiSwagger` function above) and running up our servant server.
