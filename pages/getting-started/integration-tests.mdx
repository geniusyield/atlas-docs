...
Being at the root of the framework's repository, do the following to start the private network (you would need to have `cardano-node` & `cardano-cli` in your `PATH`).

```console
cd private-testnet-simple
./scripts/automate.sh
```
...
The tests are written in {MyLink(`${exampleProjectURL}/tests-privnet/BetRef/Tests/Privnet/Tests.hs`, true, "this")} file and are being called {MyLink(`${exampleProjectURL}/tests-privnet/betref-privnet-tests.hs`, true, "here")}.
</Callout>
...
`ctx` denotes the so called context (of type `Ctx`) and contains information about our users, additional tokens, etc. It is defined in {MyLink(`${atlasRepoURL}/src/GeniusYield/Test/Privnet/Ctx.hs`, true, "Ctx.hs", true)} file and it is essential to go over that file if you intend to write these tests.
...
<Callout>
There exists a helper function in {MyLink(`${atlasRepoURL}/src/GeniusYield/Test/Privnet/Asserts.hs`, true, "Asserts.hs", true)} for what we did in the last step, i.e., to see that our expected value matches the actual considering fees, here is its definition:
...
Notice that we try catching the error using `assertThrown` function. Here `isTxBodyErrorAutoBalance` is defined as (both this & `assertThrown` have their definitions in {MyLink(`${atlasRepoURL}/src/GeniusYield/Test/Privnet/Asserts.hs`, true, "Asserts.hs", true)} file):
