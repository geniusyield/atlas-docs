...
import { exampleProjectURL } from 'config/constants'
import MyLink from 'components/my-link'

# Our Smart Contract

Let's start by writing up our Smart Contract which we'll use later to build transactions for. Though this smart contract is somewhat silly but is powerful enough to illustrate most of our framework's features.

<Callout type="warning">

  This contract is only for illustrative purposes. It is not expected to be used in production and contains security risks[^1].
...
Entire code for the contract is available {MyLink(`${exampleProjectURL}/onchain/BetRef/OnChain/BetRef.hs`, true, "here")}. This contract is inspired by the one {MyLink("https://github.com/mlabs-haskell/plutus-simple-model/blob/main/psm/test/Suites/Plutus/Model/Script/V2/Onchain/Oracle.hs", true, "available")} at MLabs repository.

### Contract's Parameters

* **`brpOraclePkh :: PubKeyHash`**: Idea here is that, our contract will use a reference input, whose datum will give us the actual result (the number of goals). This reference input UTxO must belong to Oracle and we check it using Oracle's payment public key hash.
* **`brpBetUntil :: POSIXTime`**: Time (in posix) until which bets can be placed.
* **`brpBetReveal :: POSIXTime`**: Time (again in posix) at which Oracle will reveal the correct match result.
* **`brpBetStep   :: Value`**: Each newly placed bet must be more than the previous bet by this amount.

Thus, our parameter for the contract is given by:
...
### Reference Input's Datum

Answer given by the Oracle is the number of goals scored by the team in question, this is represented in contract as:
...
Contract's datum consists of two fields:

1. List containing each person's guess with their payment public key hash. Key hash is used to tie guess with the guesser. Every time a new guess is made, we prepend it to this list.
...
2. Amount denoting the previously placed bet. Note that the total value in the concerned UTxO is a culmination of **all** the previously placed bets and thus isn't in general equal to last placed bet. We use this to see that the newly placed bet is more than the previous one by `brpBetStep` amount.
...
3. The current bet must be more than the previous bet by atleast `brpBetStep` amount.
