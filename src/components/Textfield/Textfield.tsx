...
const Textfield: FC<TextFieldProps> = ({ color = "primary", size = "medium", ...props }): ReactElement => (
  <TextFieldStyled color={color} size={size} {...props} />
);

const TextFieldStyled = styled(({ color, size, ...props }: TextFieldProps) => <MuiTextField {...props} />)(
  ({ theme, color, size }) => ({
    "& .Mui-error fieldset": {
