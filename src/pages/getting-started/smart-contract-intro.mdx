...
A group of friends want to bet on a sports game and the scores of each respective team. The winner is determined by the participant who guesses the closest score. In the case of a tie, the winner is the one who placed the earliest bet. 

The smart contract code is available [here](https://github.com/geniusyield/atlas-examples/tree/main/bet-ref/onchain/BetRef/OnChain/BetRef.hs)

This example was inspired by [MLabs](https://github.com/mlabs-haskell/plutus-simple-model/blob/main/psm/test/Suites/Plutus/Model/Script/V2/Onchain/Oracle.hs).

### Contract Parameters

* **`brpOraclePkh :: PubKeyHash`**: We'll be using a reference input, and its datum will give us the actual result (the number of goals). Since the reference input UTxO must belong to Oracle, we will read from it using the Oracle's payment public key hash.
* **`brpBetUntil :: POSIXTime`**: Time that bets can be placed.
* **`brpBetReveal :: POSIXTime`**: Time that the Oracle will reveal the match results.
* **`brpBetStep :: Value`**: Minimum number that bets must increase by.
...
The Oracle tells us the number of goals scored by the specified team:
