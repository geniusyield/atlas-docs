...
* Leverage an intuitive API to abstract away the complexity around building transactions, balancing UTxOs, and interfacing with Plutus smart contracts.

### Leverage first-class Haskell
* Avoid code duplication between on-chain and off-chain code.
* Interoperate with advanced functionalities offered by IOG's Cardano/Plutus libraries.
* Easily convert between Atlas and Cardano/Plutus types.
...
### Test extensively
* Use Atlas' test harness to write realistic [unit tests](./getting-started/unit-tests) that correspond to on-chain behavior.
* Execute [integration tests](./getting-started/integration-tests) in a private network.
